		...
		try{
			...
			for(Reuniao r : lista) {
				...
				for(Participante p : r.getParticipantes())	{
					datastr = r.getDatahora().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"));
					model.addRow(new String[]{String.format("%d", r.getId()) , datastr, r.getAssunto(), p.getNome()});
				}
			}
